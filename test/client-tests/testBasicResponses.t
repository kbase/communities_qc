...
my @func_calls = qw (get_drisee_compute get_drisee_instance get_histogram_compute get_histogram_instance get_kmer_compute get_kmer_instance);
my $func_calls = {
...
#############################################################################
my $n_tests = (scalar(@func_calls)+3); # set this to be the number of function calls + 3
...
use Server;
my ($pid, $url) = Server::start('Microbial_Communities_QCClient');
print "-> attempting to connect to:'".$url."' with PID=$pid\n";
my $client = Microbial_Communities_QCClient->new($url);
