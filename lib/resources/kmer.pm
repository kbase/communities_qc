		  ...
		  'requests' => [ { 'name'        => "info",
				  ...
				  { 'name'        => "compute",
				    ...
				    'parameters'  => { 'options' => { 'direct_return' => [ [ 0, 'always return a status token' ],
								      ...
								      'kmer_length'   => [ "integer", "lenght of the kmers, default is 15" ] },
...
sub attributes {
  ...
  return {  "id"       => [ "string" , "Unique ID of the sequence file calculated the drisee error from" ],
	    ...
	    'profile'  => [ "list of list of integer", "profile data values" ] }; 
}
...
sub instance {
  ...
  my $ex = 'curl -s -X GET '.$auth.'"'.$Conf::shock_url.'?query&type=kmer&id='.$id.'"';
  my $data = `$ex`;
  ...
  # params
  my $len = $query_params->{kmer_length} ? $query_params->{kmer_length} : 15;
  ...
  if (($rest->[1] && $rest->[1] eq 'compute') || ($params->{submethod} && $params->{submethod} eq 'compute')) {
    ...
    my $compute = submit({ resource => 'kmer',
			   id => $id,
			   token => $params->{token},
			   template => &template($len, $shock, $node_id, $id, $user) });
...
sub template {
  ...
  return {
	  ...
	  "tasks" => [
		      ...
		      {
		       "cmd" => {
				 "args" => "-l $len -i \@seqs.fasta -o seqs.stats -r",
				 "description" => "create a kmer profile from a sequence file",
				 "name" => "kmer-tool"
		      ...
		      },
		      {
		       "cmd" => {
				 ...
				 "description" => "convert kmer profile to JSON",
				 "name" => "kmer2json"
				},
		       "dependsOn" => [ "1" ],
		       "inputs" => {
