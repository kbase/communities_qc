  ...
  my $content = { 'name' => name(),
		  ...
		  'requests' => [ { 'name'        => "info",
				  ...
				  { 'name'        => "compute",
				    ...
				    'description' => "creates a sequence histogram of a metagenomic sequence",
				    'method'      => "POST" ,
				    'type'        => "asynchronous" ,  
				    'parameters'  => { 'options' => { 'direct_return' => [ [ 0, 'always return a status token' ],
											   [ 1, 'if the compute is done, return the result data' ] ],
								      'basepairs' => [ "integer", "maximum number of basepairs to analyzed, default 100" ],
								      'sequences' => [ "integer", "maximum number of sequences to be analyzed, default 100000" ]},
...
sub attributes {
  ...
  return {  "id"       => [ "string" , "Unique ID of the sequence file calculated the drisee error from" ],
	    "version"  => [ "string" , "Version of the histogram"],
	    "created"  => [ "date" , "Creation time for this histogram resource" ],
	    'counts'   => [ "array of string", "column headers" ],
	    'percent'  => [ "list of list of integer", "profile data values" ] }; 
}
...
sub instance {
  ...
  my $ex = 'curl -s -X GET '.$auth.'"'.$Conf::shock_url.'?query&type=histogram&id='.$id.'"';
  my $data = `$ex`;
  ...
  $usable_id =~ s/\|/_/;
  my $format    = 'fasta';
  my $max_bp    = $query_params->{basepairs} ? $query_params->{basepairs} : 100;
  my $max_seq   = $query_params->{sequences} ? $query_params->{sequences} : 100000;
  ...
  if (($rest->[1] && $rest->[1] eq 'compute') || ($params->{submethod} && $params->{submethod} eq 'compute')) {
    ...
    my $compute = submit({ resource => 'histogram',
			   id => $id,
			   token => $params->{token},
			   template => &template($max_bp, $format, $max_seq, $shock, $node_id, $id, $user) });
  ...
  return return_data({"ERROR" => "the id '$id' has not been computed"}, 404);
}
...
sub template {
  my ($max_bp, $format, $max_seq, $shock, $node, $id, $user) = @_;
  ...
  return {
	  ...
	  "tasks" => [
		      ...
		      {
		       "cmd" => {
				 "args" => "-v -b $max_bp -t $format -s $max_seq -i \@seqs.fasta -o seqs.stats",
				 "description" => "create a histogram from a sequence file",
				 "name" => "consensus"
				},
		       "dependsOn" => [ "0" ],
		       "inputs" => {
		      ...
		      {
		       "cmd" => {
				 "args" => "-file \@seqs.stats -id $id",
				 "description" => "convert histogram to JSON",
				 "name" => "histogram2json"
				},
		       "dependsOn" => [ "1" ],
		       "inputs" => {
