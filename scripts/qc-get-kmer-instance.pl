...
sub usage {
  print "qc-get-kmer-instance.pl >>> retrieve a kmer profile for a metagenome from the communities API\n";
  print "qc-get-kmer-instance.pl -id <id of the metagenome>\n"; 
}

sub help {
  my $text = qq~NAME

qc-get-kmer-instance -- retrieve a kmer profile for a metagenome from the communities API
...
qc-get-kmer-instance [-h] -id metagenome_id

DESCRIPTION

Retrieve a kmer profile for a metagenome from the communities API

PARAMETERS

-id metagenome_id

ID of the metagenome to retrieve the kmer profile for
...
qc-get-kmer-instance -id "kb|mg.1"
...
my $HOST      = 'http://kbase.us/services/communities_qc/kmer/';
my $id        = '';
my $user      = '';
my $pass      = '';
my $token     = '';
